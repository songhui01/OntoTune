...
def _arm_idx_to_hints(arm_idx):
    ...
    if arm_idx == 0:
        ...
        raise OntoException("RegBlocker only supports the first 5 arms")
    return hints
...
class ExperimentRunner:
    ...
    def __get_pg_cursor(self):
        try:
            ...
            raise OntoException("Could not connect to PG database") from e
        
    def add_experimental_query(self, sql):
        sql = sql.strip()
        if not sql.upper().startswith("SELECT"):
            raise OntoException("Experiment queries must be SELECT queries.")
    ...
    def test_connection(self):
        with self.__get_pg_cursor() as cur:
            try:
                ...
                raise OntoException("Could not connect to the PostgreSQL database.") from e
        return True
    ...
    def explore(self, time_limit):
        ...
        with self.__get_pg_cursor() as c:
            ...
            c.execute("SET onto_include_json_in_explain TO on")
            c.execute("SET enable_onto TO on")
            c.execute("SET enable_onto_selection TO off")
            c.execute("SET enable_onto_rewards TO on")
            c.execute("commit")
            ...
            for experiment in unexecuted:
                ...
                # get the Onto plan JSON so we can record a timeout if there is one
                c.execute("EXPLAIN (FORMAT JSON) " + sql)
                explain_json = c.fetchall()[0][0]
                
                onto_props, _qplan = explain_json

                onto_plan = json.loads(onto_props["Onto"]["Onto plan JSON"])
                onto_buffer = json.loads(onto_props["Onto"]["Onto buffer JSON"])
                ...
                try:
                    ...
                    onto_plan["Buffers"] = onto_buffer
                    storage.record_reward(onto_plan, 2 * self.__max_query_time,
                                          pid)
                    c.execute("rollback")
                except psycopg2.OperationalError as e:
                    ...
                    if arm_idx != 0:
                        ...
                        storage.record_reward(onto_plan, 2 * self.__max_query_time,
                                              pid)
                    raise OntoException(f"Server down after experiment with arm {arm_idx}") from e
...
def compute_regressions(onto_reg):
    total_regressed = 0
    total_regression = 0
    for plan_group in storage.experiment_results():
        ...
        if onto_reg:
            selection = onto_reg.predict(plans).argmin()
        else:
            # If onto_reg is false-y, compare against PostgreSQL.
            selection = 0
